...
connector = mysql.connector.connect(
    ...
    password="Mris@1665", # your MySQL password
    database="PATIENTS_DB"   # existing database
)
...
def book_appointment():
   ...
   def add_record():
      ...
      if not contact.isdigit() or len(contact) != 10:
          mb.showerror('Error!', "Phone number must be exactly 10 digits.")
        
      if DOB < date.today():
          mb.showerror("Error!", "You cannot select a past date for the appointment.")
    
      else:
          try:
              cursor.execute(
              'INSERT INTO APPOINTMENT_MANAGEMENT (NAME, EMAIL, PHONE_NO, GENDER, DOB, STREAM) VALUES (%s,%s,%s,%s,%s,%s)', (name, email, contact, gender, DOB, stream)
              )
              connector.commit()
   ...
   Label(main, text="Kapoor Hospitals", font=headlabelfont, bg='SpringGreen').pack(side=TOP, fill=X)
